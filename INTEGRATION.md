...
## Snail portraits (sprites)

Ching's 5 illustrations ship in `assets/snails/` (converted to 256px PNG). They
render as **portraits** on a dark tile — the detail screen hero and the harness
list — so the black-background ones blend in. They are NOT the in-game flying
sprite (the game stays code-drawn so it recolors per hatched snail).

- `src/minigames/snailSprites.ts` maps them and picks one per snail.
- `portraitForSnail(snail.id)` returns a stable portrait. Demo snails use ids
  matching the keys (compsci / redbull / train / businessman / graduate) so each
  shows its own art; real hatched snails (random ids) get a deterministic pick.
- To make a hatched snail show a *specific* portrait later, add a sprite/type
  field to the Snail and turn `portraitForSnail` into a direct lookup.

To swap art: drop a new PNG in `assets/snails/` and update the import in
`snailSprites.ts`. `src/minigames/sprites.d.ts` lets TS resolve `*.png` imports.
